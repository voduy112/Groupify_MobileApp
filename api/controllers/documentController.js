...
const documentController = {
  ...
  uploadDocument: async (req, res) => {
    try {
      ...
      console.log(error);
      res.status(500).json({ error: "Lỗi khi upload tài liệu" });
    }
  },
  ...
  reportDocument: async (req, res) => {
    ...
    uploadDocument: async (req, res) => {
      try {
        ...
        res.status(500).json({ error: 'Lỗi khi tải tài liệu mới', detail: error.message });
      }
    },
    ...
    deleteDocumentsByGroupId: async (req, res) => {
      ...
      try {
        ...
        for (const doc of documents) {
          if (doc.mainFile) {
            const filePublicId = doc.mainFile.split("/").slice(-3).join("/").replace(/\.(pdf)$/i, "");
            await cloudinary.uploader.destroy(filePublicId, { resource_type: "raw" });
          }
  
          if (doc.imgDocument) {
            const imgPublicId = doc.imgDocument.split("/").slice(-3).join("/").replace(/\.(jpg|jpeg|png|webp)$/i, "");
            await cloudinary.uploader.destroy(imgPublicId);
          }
    ...
    deleteComment: async (req, res) => {
      ...
      try {
        ...
        const comment = document.comments.find((c) => c._id.toString() === commentId);
